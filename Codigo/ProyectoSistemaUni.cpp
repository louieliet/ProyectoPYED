...
string showAlumnos(string pCaller) {
    ...
    string lRes = fnGetTemplate("alumno.html");
    string lHTML = "";
    PDNODO2 lData = NULL;
    int lRow = 0;
    string lColor = "";
...
string alumnData(string pCaller) {
    string lRes = fnGetTemplate("alumno.html");
    string inscr = sob.classes.htmlrepr(genUS);
    string avail = sob.MeterMateriashtml();
    strReplace(lRes, "{{pUserID}}", (genUS));
    strReplace(lRes, "{{pClasesAlumno}}", inscr);
    strReplace(lRes, "{{pClasesInscribir}}", avail);
    return lRes;
}

string profData(string pCaller) {
    string lRes = fnGetTemplate("prof.html");
    string clases = sob.mostrarClasesProfhtml(genUS);
    strReplace(lRes, "{{pUserID}}", (genUS));
    strReplace(lRes, "{{pClasesProf}}", clases);
    return lRes;
}

string alumnLogin(string pCaller) {
    string lRes = fnGetTemplate("lalumno.html");

    map<string, string> lParams = fnGetParams(pCaller);
    string pass = lParams["UsuarioContr"];
    string uss = lParams["UsuarioID"];
    cout << endl << endl << endl << "COSAAAA - ";
    cout << "id get " << " >> " << uss << "\n";
    cout << "contr get " << " >> " << pass << "\n";
    cout << endl << endl << endl;
    if (sob.alumn.checkValid(uss, pass)) {
        genUS = uss;
        strReplace(lRes, "{{pResponse}}", "");
        lRes = alumnData(pCaller);
    }
    else {
        strReplace(lRes, "{{pResponse}}", "Datos inválidos");
    }
    return lRes;
}

string profLogin(string pCaller) {
    string lRes = fnGetTemplate("lprof.html");
    ...
    cout << "contr get " << " >> " << pass << "\n";
    cout << endl << endl << endl;
    if (sob.prof.checkValid(uss, pass)) {
        genUS = uss;
        strReplace(lRes, "{{pResponse}}", "");
        lRes = profData(pCaller);
    }
    else {
        strReplace(lRes, "{{pResponse}}", "Datos inválidos");
    }
...
string Wmain(string pCaller) {
    string lRes = fnGetTemplate("main.html");
    strVec lPath = fnGetPath(pCaller);
    if (lPath.size() > 1) {
        if (lPath[1] == "LoginAlumno") {
            lRes = alumnLogin(pCaller);
        }
        else if (lPath[1] == "LoginProf") {
            lRes = profLogin(pCaller);
        }else if (lPath[1] == "AlumnoInfo") {
            lRes = alumnData(pCaller);
        }
        cout << "got -> " << lPath[1] <<endl;
    }
    return lRes;
}
            ...
            if(!sob.admi.AdminValid(uss, pas)){
                ...
                do {
                    ...
                    switch(subs){
                        ...
                        case 8:lSrv.SetGetFunc(&alumnLogin); lSrv.Run(); break;
