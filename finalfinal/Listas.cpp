...
string DLIndex::htmlrepr(bool pRev) //solo imprime los datos guardados en la lista, ya sea con frecuencia o sin ella
{
    string build = "";
    if (aHead) {
        PDNODE lTemp = (pRev == false ? aHead : aTail);
        while (lTemp) {
            build+= " <tr> ";
            build += "<td>"+lTemp->sData->sNombre + "</td>";
            build += "<td>" + lTemp->sData->sID + "</td>";
            build += "<td>" + lTemp->sData->sProfesor + "</td>";
            build += "<td>" + lTemp->sData->sSalon + "</td>";
            build += "<td>" + lTemp->sData->sDisponibilidad + "</td>";
            /*if (aFrec)
                cout << " #" << lTemp->sFrec;*/
            build += " </tr> ";
            lTemp = (pRev == false ? lTemp->sNext : lTemp->sPrev);
...
string DList::htmlrepr(ECampos pCampo, bool pRev) //solo imprime los datos guardados en la lista, ya sea con frecuencia o sin ella
{
    string build = "";
    if (aHead) {
         build = aIID->htmlrepr(pRev); 
...
string DList::reprprofhtml(string pID) {
    string build = "";
    if (aHead) {
        PDNODE lTemp = (aHead);
        while (lTemp) {
...
string aClase::MeterMateriashtml() {
    return aux.htmlrepr(ECampos::id);
}
...
string aClase::mostrarClasesProfhtml(string pID) {
    return aux.reprprofhtml(pID);
}
